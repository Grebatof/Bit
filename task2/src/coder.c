...
uint32_t decode(const CodeUnits *code_units) {
  ...
  if (code_units->length == 3) {
    return (((((code_units->code[0] - 224) << 6) + code_units->code[1] - 128) << 6) + code_units->code[2] - 128);
  }
  if (code_units->length == 4) {
    return (((((((code_units->code[0] - 240) << 6) + code_units->code[1] - 128) << 6) + code_units->code[2] - 128) << 6) + code_units->code[3] - 128);
  }
...
int name(int size, CodeUnits *code_units, FILE *in) {
  ...
  for (int i = 1; i < size; i++) {
    ...
    if (code_units->code[i] > 191 || code_units->code[i] < 128) return -1;
  }
  return 0;
}
...
int read_next_code_unit(FILE *in, CodeUnits *code_units) {
  ...
  if (k > 240) {
    code_units->code[0] = k;
    if (name(4, code_units, in) == -1) return -1;
  }
  if (240 > k && k > 224) {
    code_units->code[0] = k;
    if (name(3, code_units, in) == -1) return -1;
  }
  if (224 > k && k > 192) {
    code_units->code[0] = k;
    if (name(2, code_units, in) == -1) return -1;
  }
  if (k < 128) {
    code_units->length = 1;
    code_units->code[0] = k;
  }
  if ((193 > k && k > 127) || k == 224 || k == 240) return -1;
